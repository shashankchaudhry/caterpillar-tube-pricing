...
#    data['day'] = data.quote_date.dt.day
    ...
    data['dsq']=pow(data['diameter'],2); 
    data['wtqty'] = data['weight']/data['quantity'];
    data['length_quant'] = data['length']/data['quantity'];
...
def evalModelLabel(train_data, eval_data, train_labels, eval_labels):
    params = {}
    ...
    xgb.train(plst, xgtrain, num_boost_round=3800, evals=evallist,feval=evalerror)
...
def evalModelOHE(train_data, eval_data, train_labels, eval_labels):
    ...
    xgeval = xgb.DMatrix(eval_data,label=eval_labels)
    evallist  = [(xgeval,'eval'), (xgtrain,'train')]
    xgb.train(plst, xgtrain, num_boost_round=3000, evals=evallist,feval=evalerror)
    
def buildModelLabel(train_data, eval_data, train_labels, seed):
    print('seed: %d' % seed)
#    params = {}
#    params["seed"]=seed
#    params["objective"] = "reg:linear"
...
#    model = xgb.train(plst, xgtrain, num_boost_round=4000, feval=evalerror)
...
def buildModelOHE(train_data, eval_data, train_labels, seed):
    np.random.seed(seed)
    model = Sequential()
    model.add(Dense(train_data.shape[1], 1600))
    model.add(Activation('relu'))
    model.add(Dropout(0.2))
    model.add(Dense(1600, 1600))
    model.add(Activation('relu'))
    model.add(Dropout(0.2))
    model.add(Dense(1600, 1))
    
    model.compile(loss='mse', optimizer='rmsprop')
    
    # train model, test on 10% hold out data
    model.fit(train_data, train_labels, batch_size=100, nb_epoch=100, verbose=2, validation_split=0.1)
    preds = np.expm1(model.predict(eval_data, verbose=0).flatten())
...
#    print('training data...')
#    evalModel(train_set, eval_set, train_label, eval_cost)
    
    # get eval indices
    train_data = data.loc[data.id < 0,:]
    test_data = data.loc[data.id > 0,:]
    n_folds = 1
    all_eval_ids = getEvalSets(train_data, n_folds)
    classifiers = [(True,buildModelOHE)]
    
    dataset_blend_test = np.zeros((test_data.shape[0], n_folds * len(classifiers)))
    
    print("Making classifier based data...\n")
    
    for j, clf in enumerate(classifiers):
        print('classifier no: %d' % j)
        dataset_blend_test_j = np.zeros((test_data.shape[0], len(all_eval_ids)))
        (train_arr, test_arr, train_y, eval_ids, all_columns) = preprocessData(train_data,test_data,comp_wts_all,is_one_hot_encoding=classifiers[j][0])
        xgtest = xgb.DMatrix(test_arr)
        for i, eval_id in enumerate(range(n_folds)):
            print("Seed", 10 * (i+1) + (j+1))
            (model,y_submission) = classifiers[j][1](train_arr, test_arr, train_y, 10 * (i+1) + (j+1))
            dataset_blend_test[:, n_folds * j +  i] = y_submission
    
    median_pred = np.median(dataset_blend_test,axis=1)
    mean_pred = np.mean(dataset_blend_test,axis=1)
    gm_pred = sp.stats.gmean(dataset_blend_test,axis=1)
    #hm_pred = sp.stats.hmean(dataset_blend_test,axis=1)
    median_pred = np.expm1(median_pred)
    mean_pred = np.expm1(mean_pred)
    gm_pred = np.expm1(gm_pred)
    #hm_pred = np.expm1(hm_pred)
    for i in range(median_pred.shape[0]):
        ...
        #if hm_pred[i] < 0:
         #   hm_pred[i] = 0
    median_preds = pd.DataFrame({"id": eval_ids, "cost": median_pred})
    #median_preds.to_csv('./data/results/result_one_hot_08-27-15_median_keras.csv', index=False)
    mean_preds = pd.DataFrame({"id": eval_ids, "cost": mean_pred})
    mean_preds.to_csv('./data/results/result_one_hot_08-27-15_mean_keras_2.csv', index=False)
    gm_preds = pd.DataFrame({"id": eval_ids, "cost": gm_pred})
    gm_preds.to_csv('./data/results/result_one_hot_08-27-15_gm_keras_2.csv', index=False)
    #hm_preds = pd.DataFrame({"id": eval_ids, "cost": hm_pred})
    #hm_preds.to_csv('./data/results/result_one_hot_08-27-15_hm_keras.csv', index=False)
