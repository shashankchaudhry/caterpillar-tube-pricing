...
def buildModelOHE(train_data, eval_data, train_labels, seed):
    ...
    xgtrain = xgb.DMatrix(train_data,label=train_labels)
    xgeval = xgb.DMatrix(eval_data)
    model = xgb.train(plst, xgtrain, num_boost_round=3200, feval=evalerror)
    preds = model.predict(xgeval)
    preds = np.expm1(preds)
...
#    evalModel(train_set, eval_set, train_label, eval_cost)
    ...
    classifiers = [(True,buildModelOHE),(False,buildModelLabel)]
    ...
    median_preds.to_csv('./data/results/result_one_hot_08-27-15_median.csv', index=False)
    mean_preds = pd.DataFrame({"id": eval_ids, "cost": mean_pred})
    mean_preds.to_csv('./data/results/result_one_hot_08-27-15_mean.csv', index=False)
    gm_preds = pd.DataFrame({"id": eval_ids, "cost": gm_pred})
    gm_preds.to_csv('./data/results/result_one_hot_08-27-15_gm.csv', index=False)
    hm_preds = pd.DataFrame({"id": eval_ids, "cost": hm_pred})
    hm_preds.to_csv('./data/results/result_one_hot_08-27-15_hm.csv', index=False)
