...
def evalModel(train_data, eval_data, train_labels, eval_labels):
    model = Sequential()
    model.add(Dense(train_data.shape[1], 5000))
    model.add(Activation('relu'))
    #model.add(Dropout(0.2))
    model.add(Dense(5000, 5000))
    model.add(Activation('relu'))
    #model.add(Dropout(0.2))
    model.add(Dense(5000, 1))
    
    model.compile(loss='mse', optimizer='rmsprop')
    
    # train model, test on 15% hold out data
    model.fit(train_data, train_labels, batch_size=320, nb_epoch=100, verbose=2, validation_data=(eval_data,eval_labels))
    return(model)
