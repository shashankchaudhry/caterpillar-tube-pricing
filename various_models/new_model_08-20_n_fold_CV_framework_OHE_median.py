...
#    evalModel(train_set, eval_set, train_label, eval_cost)
    ...
    dataset_blend_train = np.zeros((train_data.shape[0], len(classifiers)))
    dataset_blend_test = np.zeros((test_data.shape[0], len(classifiers)))
    ...
    for j, clf in enumerate(classifiers):
        ...
        for i, eval_id in enumerate(all_eval_ids):
            ...
            (model,y_submission) = classifiers[j][1](X_train_set, X_eval_set, y_train, 10 * (i+1) + (j+1))
    ...
    print("Blending")
    clf = SVR()
    clf.fit(dataset_blend_train, np.log1p(np.array(train_y)))
...
#    preds.to_csv('./data/results/result_one_hot_08-22-15_piece1.csv', index=False)
