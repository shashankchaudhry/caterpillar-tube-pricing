...
for i in range(num_runs):
    print('test run: %d' % i)
    clf = RandomForestRegressor(n_estimators=100,n_jobs=-1,min_samples_leaf=11)
...
feat_arr = np.array(feat_imp[0])
feat_arr = np.add(feat_arr, np.array(feat_imp[1]))
feat_arr = np.add(feat_arr, np.array(feat_imp[2]))

feat_name_arr = np.array(out_columns)
ind = np.argpartition(feat_arr,-4)
feat_name_arr[ind[-10:][::-1]]
