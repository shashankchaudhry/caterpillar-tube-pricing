...
def simpleMeanEncodeColSingle(data, col_name, out_col_name):
    print('simple mean encoding : %s' % col_name)
    train = data.ix[data['id'] < 0,:]
    ...
    overall_mean = train[out_col_name].mean()
    overall_sdev = train[out_col_name].std()
...
#    temp_flag = temp_flag.values
#    for i in range(data.shape[0]):
#        if temp_flag[i]:
#            tmp_quantity = data.ix[i,'quantity']
#            reqd_series = train.ix[train['quantity'] == tmp_quantity,'cost']
#            data.ix[i,'mean_' + col_name] = reqd_series.mean()
#            #data.ix[i,'sdev_' + col_name] = reqd_series.std()
...
def get_cross_val_score(train, label, ss, num_runs):
    ...
    for i in range(n_runs):
        clf = xgb.XGBRegressor(n_estimators = 800, seed=i, learning_rate= 0.3, max_depth=3,min_child_weight=1,\
                        gamma=1,subsample=0.9,colsample_bytree=0.1)
...
for i in range(num_runs):
    print('test run: %d' % i)
    clf = xgb.XGBRegressor(n_estimators = 800, seed=i, learning_rate= 0.3, max_depth=7,min_child_weight=1,\
                        gamma=1,subsample=0.8,colsample_bytree=0.1)
...
preds.to_csv('./data/results/result_python_add_tube_assem_id_08-18-15_sub2.csv', index=False)