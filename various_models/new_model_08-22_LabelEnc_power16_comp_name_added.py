...
def get_rmsle(y_pred, y_actual):
    y_pred = np.power(y_pred,16.0)
    y_actual = np.power(y_actual,16.0)
    mean_error = np.sqrt(np.mean((np.log1p(y_pred)-np.log1p(y_actual))**2))
    return(mean_error)
...
def preprocessData(train_data, eval_data, comp_wts_all):
    ...
    train_label = np.power(np.array(train_data['cost']),1.0/16.0)
...
def evalModel(train_data, eval_data, train_labels, eval_labels):
    params = {}
    ...
    xgb.train(plst, xgtrain, num_boost_round=1500, evals=evallist,feval=evalerror)
...
if __name__ == "__main__":
    ...
    comp_name_df=pd.read_csv('./data/competition_data/components.csv',usecols=['component_id','name'])
    comp_name_df=comp_name_df.set_index(['component_id'])
    comp_wts_all=comp_wts_all.join(comp_name_df,on='component_id')
    ...
    eval_cost = np.power(np.array(eval_set['cost']),1.0/16.0)
    eval_set['cost'] = 0.
