...
def buildModelLabel(train_data, eval_data, train_labels, seed):
    ...
    model = xgb.train(plst, xgtrain, num_boost_round=3800, feval=evalerror)
...
def buildModelOHE(train_data, eval_data, train_labels, seed):
    """
    Parameters: {'n_estimators': 3668, 'subsample': 0.512678980363821, 
    'learning_rate': 0.0129345512066281, 'max_delta_step': 1.0473526885498312, 
    'max_depth': 10}
    """
    print('seed: %d' % seed)
    params = {}
    ...
    xgtrain = xgb.DMatrix(train_data,label=train_labels)
    xgeval = xgb.DMatrix(eval_data)
    model = xgb.train(plst, xgtrain, num_boost_round=5000, feval=evalerror)
    preds = model.predict(xgeval)
    preds = np.expm1(preds)
...
#    print('training data...')
#    evalModelOHE(train_set, eval_set, train_label, eval_cost)
    ...
    for j, clf in enumerate(classifiers):
        ...
        for i, eval_id in enumerate(range(n_folds)):
            ...
            (model,y_submission) = classifiers[j][1](train_arr, test_arr, train_y, 10 * (i+1) + (j+1) + 3)
            print(y_submission[:50])
    ...
    median_preds.to_csv('./data/results/result_one_hot_08-31-15_median.csv', index=False)
    mean_preds = pd.DataFrame({"id": eval_ids, "cost": mean_pred})
    mean_preds.to_csv('./data/results/result_one_hot_08-31-15_mean.csv', index=False)
    gm_preds = pd.DataFrame({"id": eval_ids, "cost": gm_pred})
    gm_preds.to_csv('./data/results/result_one_hot_08-31-15_gm.csv', index=False)
    hm_preds = pd.DataFrame({"id": eval_ids, "cost": hm_pred})
    hm_preds.to_csv('./data/results/result_one_hot_08-31-15_hm.csv', index=False)
