...
if __name__ == "__main__":
    ...
    dataset_blend_train = np.zeros((train_data.shape[0], n_seeds * len(classifiers)))
    dataset_blend_test = np.zeros((test_data.shape[0], n_seeds * len(classifiers)))
    ...
    for j, clf in enumerate(classifiers):
        ...
        for k in range(n_seeds):
            print('seed no: %d' % k)
            for i, eval_id in enumerate(all_eval_ids):
                print("Fold", i)
                test_val = np.array(train_data['tube_assembly_id'].isin(eval_id))
                train_val = ~test_val
                X_eval_set = train_arr[test_val,:]
                X_train_set = train_arr[train_val,:]
                y_test = train_y[test_val]
                y_train = train_y[train_val]
                (model,y_submission) = classifiers[j][1](X_train_set, X_eval_set, y_train, 100*(i+1) + 10*(k+1) + (j+1))
                dataset_blend_train[test_val, n_seeds*j + k] = y_submission
                temp_pred = model.predict(xgtest)
                dataset_blend_test_j[:, i] = temp_pred
            dataset_blend_test[:,n_seeds*j + k] = dataset_blend_test_j.mean(1)
    
    print(dataset_blend_train)
    print(dataset_blend_test)
    print("Blending")
    pickle.dump( dataset_blend_train, open( "./data/pickle/RF_top_5_train.pkl", "wb" ) )
    pickle.dump( dataset_blend_test, open( "./data/pickle/RF_top_5_test.pkl", "wb" ) )
    clf = SVR()
    ...
    print(str(y_submission.shape))
    print(str(eval_ids.shape))
    preds = pd.DataFrame({"id": eval_ids, "cost": y_submission})
    preds.to_csv('./data/results/result_one_hot_08-24-15_ensemble.csv', index=False)
